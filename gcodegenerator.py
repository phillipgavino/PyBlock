...
def gcode(filename, points):
    ...
    # origin (0,0) is the bottom left peg on the platform (nearest where printer zeroes itself.)
    
    if not filename.endswith(".gcode"): #add .gcode to the filename if it was omitted
        filename = filename + ".gcode"
        
    # sort the points by z so lowest layers printed first, handles both numpy arrays and lists of lists
    ...
    for p in sortedpoints:
        err1 = False
        err2 = False
        if not p[0]%1 == 0.5: #make sure x axis aligns with pegs (every 0.5")
            err1 = True
        if not p[1]%1 == 0.5: #make sure y axis aligns with pegs
            err1 = True
        if not p[2]%1 == 0:   #make sure every z is an integer (no half blocks)
            err2 = True
        if err1:
    ...
    xOffset = 35.25   #mm offsets from point where printer zeroes against its limit switches to pickup point of the blocks
    yOffset = 224.75
    ...
    #header is always at the top of every gcode file, the same regardless of geometry input
    ...
    header = header + "M220 S250 ;Reset Feedrate (travel motor speed %)\n"
    header = header + "M221 S100 ;Reset Flowrate (extruder motor speed %)\n"
    ...
    with open(filename, "w") as file: #open a file in write mode
        file.write(header)  #add the header to the top

        for p in sortedpoints:  #loop over every block location
            x = xOffset - 1.5*12.7 + (p[0]*12.7) #convert coordinate grid integers to mm (12.7mm = 1/2")
            y = yOffset - 14.5*12.7 + (p[1]*12.7)
            z = zOffset + (p[2]*25.4)  #blocks are all 1" tall in z

            #placement code for each block
            block = " \n"
